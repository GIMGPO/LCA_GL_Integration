...
{
	public class Copy : TransformBase
	{
		...
		public Copy(XmlDocument doc, Transform transform, RunMode runMode, Hashtable htmlEntities, NamespaceManagerHelper nsmh, XmlDocument configDoc, Hashtable values) : base(doc, transform, runMode, htmlEntities, nsmh, configDoc, values)
		{
		}
		...
		private void ProcessNodeForward(XmlNode node)
		{
			...
			if (enclosedObject != null && IsTargetContainer(_copyTargetNodeType) && ObjectsTypesCompatible(enclosedObject, _copyTargetNodeType))
			{
				...
				if (! targetExists)
				{
					XmlHelper.AddSingleNodeAt(GetCopyParentNode(node, _copyXPath), newNode, null, true);
				}
		...
		private void PopulateNode(XmlNode workNode, XmlNode newNode, object enclosedObject)
		{
			switch (enclosedObject.GetType().FullName)
			{
				...
				case "System.Xml.XmlElement":
                    newNode.AppendChild(((XmlElement)enclosedObject).Clone());
					break;
...
//				XmlNode refNode = newNodes.Count > 0 ? newNodes[0] : null;
				...
				else if (_extraNodeType == XmlNodeType.Element)
				{
					...
					XmlHelper.AddSingleNodeAt(newNode, extraElement, null, true);
				}
			}
		}
