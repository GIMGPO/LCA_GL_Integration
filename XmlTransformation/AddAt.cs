...
{
	public class AddAt  : TransformBase
	{
		XmlNodeType _addedNodeType;
		string _addedNodeName;

		public AddAt(XmlDocument doc, Transform transform, RunMode runMode, Hashtable htmlEntities, NamespaceManagerHelper nsmh, XmlDocument configDoc, Hashtable values) : base(doc, transform, runMode, htmlEntities, nsmh, configDoc, values)
		{
		...
		private void AddElement(XmlNode node)
		{
			...
			switch (node.NodeType)
			{
                ...
                XmlHelper.AddSingleNodeAt(node, newElement, node.ChildNodes[0], GetAddAfter(Transform.Action.Extra));
					break;
		...
		private object GetValue(XmlNode node, Extra extra)
		{
			...
			if (extra != null)
			{
				string methodText = XmlHelper.GetText(extra, METHOD);
				string valueText = XmlHelper.GetText(extra, VALUE);
		...
		private bool GetRemoveWhitespace(Extra extra)
		{
			...
			string whiteSpace = XmlHelper.GetText(extra, WHITESPACE);

			return whiteSpace == null ? false : whiteSpace.ToLower() == DELETE;
		}

        private bool GetAddAfter(Extra extra)
        {
