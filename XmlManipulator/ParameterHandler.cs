...
namespace XmlManipulator
{
	public class ParameterHandler
	{
		...
		public bool ValidateParameters()
		{
			...
			{
				new BrokerException(string.Format(ResStrings.INVALID_ARG_COUNT, _parameterConfig.Min, _parameterConfig.Max, _args.Length));
				return false;
			}
			...
			for (int i = 0; i < _args.Length; ++i)
			{
				...
				{
					new BrokerException(string.Format(ResStrings.INVALID_ARG, Convert.ToString(i+1)));
					return false;
				}
				...
				{
					new BrokerException(string.Format(ResStrings.INVALID_ARG_TYPE, Convert.ToString(i+1), parameterDescriptor.Type.Name));
					return false;
				}
				...
				if (parameterDescriptor.IsFile)
				{
					...
					{
						new BrokerException(string.Format(ResStrings.INVALID_FILE, _args[i]));
						return false;
					}
				}
		...
		public object GetArgumentValue(string argName)
		{
			...
			throw new Exception(string.Format(ResStrings.ARGUMENT_MISSING, argName));
		}
		...
		public object GetArgumentValue(int argIndex)
		{
			...
			{
				throw new Exception(string.Format(ResStrings.INVALID_ARG_TYPE, parameterDescriptor.Type.Name));
			}
